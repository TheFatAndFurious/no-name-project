...
export async function insertInto(tableName: string, newInsert:string){   
        const { data: galery } = await supabase.from(tableName).insert({name: newInsert}).single()
        return galery
}
...
export default function CreateGalery( {tableName } : CreateGaleryProps){
    ...
    return (
        ...
        <input type="text" placeholder="Nom de la nouvelle galerie" value={newInsert} onChange={(e) => {setNewInsert(e.target.value)}}/>
        </>
    )
}
