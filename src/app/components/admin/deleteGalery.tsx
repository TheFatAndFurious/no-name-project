...
export default function CreateGalery(){
    ...
    async function createNew(galeryName: string) {
        const { data: galery } = await supabase.from("galeries").insert({name: galeryName}).single()
        setGaleryName("");
    }

    return (
        <>
        <button onClick={()=> createNew(galeryName) }>Creer une nouvelle galerie</button>
        <input type="text" placeholder="Nom de la nouvelle galerie" value={galeryName} onChange={(e) => {setGaleryName(e.target.value)}}/>
        </>
    )
}